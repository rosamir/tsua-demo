...
// Fetches IZ152 data from FlightStats, detects changes,
// writes public/data/*.json, and emails subscribers via Gmail.
'use strict';
...
function parseNextData(nextData) {
  ...
  // FlightStats has used several different pageProps shapes over time
  const raw =
    pp?.flightData ||
...
function buildHtml(flight, changes) {
...
<div style="max-width:580px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)">
  <div style="background:linear-gradient(135deg,#1a365d 0%,#2b6cb0 100%);padding:28px;text-align:center">
    <div style="font-size:13px;color:#90cdf4;letter-spacing:2px;text-transform:uppercase;margin-bottom:6px">עדכון טיסה</div>
    <div style="font-size:40px;font-weight:900;color:#fff;letter-spacing:4px">${flight?.flightNumber || 'IZ152'}</div>
    <div style="color:#bee3f8;margin-top:4px;font-size:14px">✈️ פאפוס ← תל אביב &nbsp;·&nbsp; 3.5.2026</div>
  </div>
  <div style="background:#ebf8ff;padding:12px;text-align:center;border-bottom:2px solid #bee3f8">
    <span style="font-weight:700;color:#2c5282;font-size:15px">סטטוס: ${flight?.statusHe || '—'}</span>
  </div>
  <div style="padding:24px">
    <div style="font-size:17px;font-weight:700;color:#1a365d;margin-bottom:14px">🔄 ${changes.length === 1 ? 'שינוי 1' : `${changes.length} שינויים`} זוהו</div>
    ...
    <table width="100%" style="margin-top:18px;border-collapse:collapse">
      <tr>
        <td style="background:#ebf8ff;border-radius:12px;padding:14px;text-align:center;width:48%">
          ...
          <div style="font-size:10px;color:#718096">משוער</div>
        </td>
        <td style="width:4%"></td>
...
async function sendEmails(flight, changes) {
  ...
  const html = buildHtml(flight, changes);
...
async function main() {
  let newData;
  try {
    const nextData = await fetchFlightStats();
    newData = parseNextData(nextData);
    if (!newData) throw new Error('Could not parse flight data from page');
    console.log('[Poll] ✓ Fetched from FlightStats');
  } catch (e) {
    console.error('[Poll] ✗ Fetch failed:', e.message);
    // Keep existing data unchanged — nothing to commit
    process.exit(0);
  }
